...
import com.romaway.common.protocol.dl.entity.StockShuoBaReplyCommentEntity;
 ...
 */
public class StockShuoBaDetailCommentProtocolCoder extends AProtocolCoder<StockShuoBaDetailCommentProtocol> {
    @Override
    protected byte[] encode(StockShuoBaDetailCommentProtocol protocol) {
        RequestCoder reqCoder = new RequestCoder();
        return reqCoder.getData();
    }

    @Override
    protected void decode(StockShuoBaDetailCommentProtocol protocol) throws ProtocolParserException {
        ResponseDecoder r = new ResponseDecoder(protocol.getReceiveData() == null ? new byte[0] : protocol.getReceiveData());
        String result = r.getString();

        Logger.d("StockShuoBaDetailCommentProtocolCoder", "decode >>> result body = " + result);
        try {
            if (!TextUtils.isEmpty(result)) {
                ...
                if (jsonObject.has("data")) {
                    ...
                    ArrayList<StockShuoBaCommentEntity> list = new ArrayList<StockShuoBaCommentEntity>();
                    int count = jsonArray.length();
                    for (int i = 0; i < count; i++) {
                        BaseJSONObject jsonObject1 = jsonArray.getJSONObject(i);
                        StockShuoBaCommentEntity entity = new StockShuoBaCommentEntity();
                        ...
                        if (jsonObject1.has("reply")) {
                            ...
                            ArrayList<StockShuoBaReplyCommentEntity> list_reply = new ArrayList<StockShuoBaReplyCommentEntity>();
                            int count2 = jsonArray1.length();
                            for (int j = 0; j < count2; j++) {
                                BaseJSONObject jsonObject2 = jsonArray1.getJSONObject(j);
                                StockShuoBaReplyCommentEntity entity1 = new StockShuoBaReplyCommentEntity();
