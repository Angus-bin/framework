 ...
 */
public class StockDetailNoticeDataProtocolCoder extends AProtocolCoder<StockDetailNoticeDataProtocol> {
    @Override
    protected byte[] encode(StockDetailNoticeDataProtocol protocol) {
        RequestCoder reqCoder = new RequestCoder();
        return reqCoder.getData();
    }

    @Override
    protected void decode(StockDetailNoticeDataProtocol protocol) throws ProtocolParserException {
        ResponseDecoder r = new ResponseDecoder(protocol.getReceiveData() == null ? new byte[0] : protocol.getReceiveData());
        String result = r.getString();

        Logger.d("StockDetailNoticeDataProtocolCoder", "decode >>> result body = " + result);
        try {
            if (!TextUtils.isEmpty(result)) {
                ...
                if (jsonObject.has("data")) {
                    ...
                    ArrayList<StockNoticeEntity> list = new ArrayList<StockNoticeEntity>();
                    int count = jsonArray.length();
                    for (int i = 0; i < count; i++) {
                        BaseJSONObject jsonObject1 = jsonArray.getJSONObject(i);
                        StockNoticeEntity entity = new StockNoticeEntity();
