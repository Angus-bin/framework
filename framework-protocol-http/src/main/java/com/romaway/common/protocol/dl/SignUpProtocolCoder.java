 ...
 */
public class SignUpProtocolCoder extends AProtocolCoder<SignUpProtocol> {
    @Override
    protected byte[] encode(SignUpProtocol protocol) {
        RequestCoder reqCoder = new RequestCoder();
        return reqCoder.getData();
    }

    @Override
    protected void decode(SignUpProtocol protocol) throws ProtocolParserException {
        ResponseDecoder r = new ResponseDecoder(protocol.getReceiveData() == null ? new byte[0] : protocol.getReceiveData());
        String result = r.getString();

        Logger.d("SignUpProtocolCoder", "decode >>> result body = " + result);
        try {
            ...
            if (jsonObject.has("xy")) {
                ...
                Logger.d("SignUpProtocolCoder", "decode >>> result body = " + DES3.decode(DES3.decodeHex(str)));
