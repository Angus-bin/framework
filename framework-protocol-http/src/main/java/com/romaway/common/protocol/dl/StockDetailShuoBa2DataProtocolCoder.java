 ...
 */
public class StockDetailShuoBa2DataProtocolCoder extends AProtocolCoder<StockDetailShuoBa2DataProtocol> {
    @Override
    protected byte[] encode(StockDetailShuoBa2DataProtocol protocol) {
        RequestCoder reqCoder = new RequestCoder();
        return reqCoder.getData();
    }

    @Override
    protected void decode(StockDetailShuoBa2DataProtocol protocol) throws ProtocolParserException {
        ResponseDecoder r = new ResponseDecoder(protocol.getReceiveData() == null ? new byte[0] : protocol.getReceiveData());
        String result = r.getString();

        Logger.d("StockDetailShuoBa2DataProtocolCoder", "decode >>> result body = " + result);
        try {
            if (!TextUtils.isEmpty(result)) {
                ...
                if (jsonObject.has("data")) {
                    ...
                    ArrayList<StockShuoBa2Entity> list = new ArrayList<StockShuoBa2Entity>();
                    int count = jsonArray.length();
                    for (int i = 0; i < count; i++) {
                        BaseJSONObject jsonObject1 = jsonArray.getJSONObject(i);
                        StockShuoBa2Entity entity = new StockShuoBa2Entity();
                        ...
                        if (jsonObject1.has("nickname")) {
                            entity.setNickname(jsonObject1.getString("nickname"));
                        }
