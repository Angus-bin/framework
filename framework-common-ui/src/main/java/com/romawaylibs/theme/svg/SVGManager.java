...
public class SVGManager {
    ...
    public static String getSVGParseContentAsStoragePath(Context context, String svgFileName){
    	String config = "";
    	if(ROMA_SkinManager.iconLoadPathType == ROMA_SkinManager.SAVE_TYPE_ASSETS)//本地存储位置
    		config = FileSystem.getFromAssets(context, SVG_ICON_PATH + svgFileName +".svg");
    	
    	else if(ROMA_SkinManager.iconLoadPathType == ROMA_SkinManager.SAVE_TYPE_SDCARD)//SDK存储位置
    		config = ""; //"file://"+FileSystem.getCacheRootDir(context, "").getPath()+ url;
    	
    	else if(ROMA_SkinManager.iconLoadPathType == ROMA_SkinManager.SAVE_TYPE_SYSTEM_DATA_FOLDER)//内部文件系统存储位置
	...
	public static InputStream getConfigStreamAsSkinType() {
		...
		return SVGManager.class.getResourceAsStream("/assets/themeFolder/"+ ROMA_SkinManager.getCurSkinType()+"/SVGConfig.properties");
	}
    ...
    public static SVGParserRenderer getSVGParserRenderer(Context context, String svgFileName){
		try {
			...
			svgContent = parseCssStyleToSvg(ROMA_SkinManager.getCurSkinType(), svgFileName, svgContent);
