...
public class TableFixHeaders extends ViewGroup {
	...
	public TableFixHeaders(Context context) {
		this(context, null);
		isGuoyuan = getResources().getBoolean(R.bool.hq_zixuan_is_show_guoyuan_dapan);
	}
			...
			case MotionEvent.ACTION_UP: {
				...
				if (Math.abs(velocityX) > minimumVelocity || Math.abs(velocityY) > minimumVelocity) {
					...
					if(mOnScrollChangedListener != null)
						mOnScrollChangedListener.onScrollFinished(scrollDir,scrollX, scrollY, firstRow, firstColumn);
...
//		scrollY += y;
		...
		if(mOnScrollChangedListener != null)
			mOnScrollChangedListener.onScrollChanged(scrollDir, scrollX, scrollY, firstRow, firstColumn);
		//System.out.println("scrollBy scrollX = "+scrollX+",scrollY = "+scrollY+",firstRow = "+firstRow);
	...
	public interface OnScrollChangedListener {
		...
		/**
		 * 滚动变化监听器,滚动过程中的监听
		 * @param scrollDir 滚动的方向，0：水平滚动， 1：垂直滚动
		 * @param scrollX 水平滚动的距离
		 * @param scrollY 垂直滚动的距离
		 * @param firstRow 显示的第一行内容索引
		 * @param firstColumn 显示的第一列内容索引
		 */
	     void onScrollChanged(int scrollDir, int scrollX, int scrollY, int firstRow, int firstColumn);
	      ...
	      * @param firstColumn
	      */
	     void onScrollFinished(int scrollDir, int scrollX, int scrollY, int firstRow, int firstColumn);
	}
}
